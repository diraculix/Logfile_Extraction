...
class MachineLog:
                    ...
                    for record_file in map_records:  # actual (processed) log-file analysis
                        if int(record_file.split('_')[2].split('_')[0]) == layer_id:
                            ...
                            while current_spot_submap <= record_file_df['SUBMAP_NUMBER'].max(): # sweep over all spots (submaps), SUBMAP_NUMBER is locked whenever a spot is active
                                record_file_df.loc[record_file_df['SUBMAP_NUMBER'] == current_spot_submap, ['SPOT_ID']] = current_spot_id  # assign new column 'SPOT_ID'
                            ...
                            record_file_df['MU'] = abs(record_file_df['DOSE_PRIM(C)'] / correction_factor / charge_per_mu)  # calculate MUs and assign new column 'MU'
    ...
    def plot_beam_layers(self):     # For all layers in one beam, averaged over all fractions:
        ...
        print('Select according plan .dcm file..')  # read RT plan dicom via filedialog
        root = Tk()
        ds = pydicom.read_file(filedialog.askopenfilename())
        root.destroy()
        
        for i, beam in enumerate(ds.IonBeamSequence):
            if beam.BeamName == beam_id or beam.BeamDescription == beam_id:  # and float(beam.IonControlPointSequence[0].GantryAngle) == self.patient_record_df.loc[self.patient_record_df['BEAM_ID'] == beam_id]['GANTRY_ANGLE'].iloc[0]:
                dcm_beam_id = i  # locate beam according to previous selection
                break
        
        dcm_beam = ds.IonBeamSequence[dcm_beam_id]
        ...
        for layer_id in scope_record_df['LAYER_ID'].drop_duplicates():
            ...
            axs[layer_id].plot(x_positions, y_positions, marker='o', markerfacecolor='None', linestyle='-', markersize=2.0, markeredgewidth=0.2, linewidth=0.2, label='Log-file mean')
